...
import {adminSignup } from "../store/action/auth";
...
let AdminSignup =() =>{
  ...
  let submitHandler = async (e)=>{
    ...
    try{
      let response = await dispatch(adminSignup({email,password,photo,secretKey}))
      if(response=='signin'){
         navigate('/adminlogin')
          ...
          <form onSubmit={submitHandler}encType="multipart/form-data" id='form'>
            ...
            <input 
             ...
             onChange={changeHandler}
              ...
              <label  className={styles.label}>secretkey</label>
              <input 
             className ={styles.input}
             type="text"
